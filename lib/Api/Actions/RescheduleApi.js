...
const RescheduleApiRequestDTO = require('./dtos/RescheduleApiRequestDTO');

function RescheduleApi(args)
{
    let url = `${args.URL}/reschedule`;

    let entity = new RescheduleApiRequestDTO(args);
    ...
    this.Validated = () => {
        ...
        if( functions.isEmpty(entity.SendTime) )
        {
            error = "Missing SendTime";
            return false;
        }
        if( !functions.isDateTime(entity.SendTime) )
        {
